...
pub const POSITION_SENSE: [&str; 6] = [
    "初爻在最下面，事情刚开头，位置低，不适合抢先",
    "二爻在下卦的中间，位置稳，容易得到帮助，可以往前走",
    "三爻是下卦的最上面，进退之间，最容易出错",
    "四爻是上卦的最下面，离主位近，容易犹疑，要看清楚上下",
    "五爻在上卦的中间，是主位，看的是有没有担当",
    "上爻在一卦的尽头，事情到头了，该收不该再进",
];
...
impl Cast {
    ...
    pub fn rule(&self) -> &'static str {
        match self.changing.len() {
            0 => "六爻都不动，看本卦的卦辞",
            1 => "一个爻动，看本卦动的那一爻",
            2 => "两个爻动，看本卦动的这两条爻，以上面那条为主",
            3 => "三个爻动，本卦与变卦一起看，以本卦为主",
            4 => "四个爻动，看变卦里不动的两条爻，以下面那条为主",
            5 => "五个爻动，看变卦里不动的那一条爻",
            _ => "六爻全动，看变卦的卦辞",
        }
    }
    ...
    fn changing_lines_flip_into_the_changed_hexagram() {
        ...
        for seed in 0..500u64 {
            ...
            if cast.changing.is_empty() {
                saw_still = true;
                assert!(cast.changed.is_none(), "没有变爻就不该有变卦");
                assert_eq!(cast.rule(), "六爻都不动，看本卦的卦辞");
                continue;
            }
            saw_changing = true;
