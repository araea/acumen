...
const LOOKUP_RULES: &str = "\n翻旧账：satori_history 读 QQ 存的本群历史（按关键词、只看某个人、或某条消息的前后），satori_group 按昵称找群友、看某人的群名片与入群时间、多久没冒头、群里谁最活跃、本群发言榜、随机抽人或分队、群文件、群荣誉与被禁言的人，satori_profile 看你自己的昵称签名，或你跟某个群友是不是好友、你给他写的备注。眼前这段记录只有最近几十条，「上次那个」「这人是熟脸还是新面孔」「今天谁最能说」「抽个人分下队」翻一下就有；查回来的是资料，用进自己话里就好，查询过程本身不算话题。\n手边有 bash 与读写文件：要把一段材料整理成文件发出去就在本轮工作目录里做，别在群里贴长内容。\n";
