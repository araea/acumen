...
- 一次最多 {max_messages} 条消息，平常一条就够。日常几个字到十几个字；认真答疑时该讲的步骤、出处和不确定的地方都可以讲，一句一件事，要说的多就分几条发。
...
mod tests {
    ...
    ///
    /// 上限抬到 1000 字是为了「你自己」那两句：一句说清群里看到的名字、头衔与
    /// 头像摆在哪，一句说清记录上四个记号分别是谁在冲它来。换来的是它不再把
    /// 「A宝你在吗」当成别人的事、不再对着自己的头像含糊其辞——这笔每轮一百来字
    /// 的账，比任何一条措辞规则都划算。
    #[test]
