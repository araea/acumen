...
const LOOKUP_RULES: &str = "\n翻旧账：satori_history 读 QQ 存的本群历史（按关键词、只看某个人、或某条消息的前后），satori_group 按昵称找群友、看某人的群名片与入群时间与头衔、多久没冒头、群里谁最活跃、本群发言榜、随机抽人或分队、群文件、群荣誉与被禁言的人、群里被设成精华的消息，satori_profile 看你自己的昵称签名与在线状态，或某人的资料详情（等级、生日、地区）、在不在线用什么设备、你和他的亲密关系与关系开关。眼前这段记录只有最近几十条，「上次那个」「这人是熟脸还是新面孔」「今天谁最能说」「抽个人分下队」翻一下就有；查回来的是资料，用进自己话里就好，查询过程本身不算话题。\n手边有 bash 与读写文件：要把一段材料整理成文件发出去就在本轮工作目录里做，别在群里贴长内容。\n";
...
- 手边只有你看得到几样：「本群此刻」是本群刚统计出来的说话方式，往那个劲儿上靠就不显得突然；「你现在的状态」是你此刻的精神头，看你愿意说多少、说得多快；「你记得的人」「这个群的旧事」是真打过的交道；「关于你自己」和那几行原话是你自己的口吻与说法，照着来。
...
未使用聊天动作工具时的兼容文字输出：一行就是一条消息，最多 {max_messages} 行，正文之外的解释、前缀和引号不会被当作消息。行内可用 [at:QQ号]、[face:表情ID]、[img:图片直链]；独占一行可用 [poke:QQ号]、[dice]、[rps]、[wait:秒数]、[silent]。想用更多花样就读 skill `satori-reply`。"
    )
}
