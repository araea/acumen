...
register_plugins!(
    ...
    ctl {
        display_name: "插件控制",
        section: "system",
        summary: "统一管理全部插件开关与配置；仅 ctl.admins 可改，初始化与排期待重启",
        commands: cmds![
            ("ctl / 控制 / 插件", "查看完整用法和示例"),
            ("ctl list [on|off|关键词]", "查看全局状态及待重启提示"),
            ("ctl on <插件…>", "批量启用，支持英文名及中文显示名"),
            ("ctl off <插件…>", "批量停用；保留 ctl 管理入口"),
    ...
    media {
        ...
        commands: cmds![
            ("转链接 / 看链接 / 提取地址 / url", "将图片或视频转为直链（可引用消息）"),
            ("转图片 / 预览", "将链接转为图片发送"),
            ("转视频", "将链接转为视频发送"),
        ]
    },
    ...
    portrait {
        display_name: "用户画像",
        section: "insight",
        summary: "读群成员历史发言，按活跃、内容、交互、表达四个维度出图文画像",
        commands: cmds![
            ("画像 / 用户画像 / 我的画像 / 人物画像 / 画像报告 / 用户画像报告", "为自己出一份用户画像"),
            ("画像 @某人 / 画像 <QQ号>", "为对方出一份用户画像"),
        ]
    },
    gif {
    ...
    video_parse {
        ...
        commands: cmds![
            ("引用预览 + 视频 / 原片 / 原视频 / 下载 / 下载视频 / 发视频 / 取片 / 文件 / video / mp4",
             "把这条链接的原片取进群（群文件 + 视频气泡）；引用 30 天内的预览都算数"),
        ]
    },
    webshot {
        display_name: "网页截图",
    ...
    oai {
        ...
        commands: cmds![
            ...
            ("##<名称> agent", "建一间内置智能体房间；模型与工具由 [oai] 配置"),
            ("oai <API地址> <密钥>", "配置模型接口"),
            ("##<名称>(<描述>) <模型> <提示词>", "创建智能体"),
            ("~<名称> <内容>", "与智能体对话"),
    ...
    ai_news {
        ...
        commands: cmds![
            ...
            ("ai日报", "最新一期 AI 日报"),
            ("ai模型排行榜 / ai模型榜 / 模型排行榜 / 模型榜", "AIHOT 大模型排行榜：共识分、评测完整度与官网参考价"),
            ("ai搜索 <关键词>", "近 7 天按关键词检索 AI 资讯"),
            ("ai推送添加 <群|私聊> <ID>", "从任意群或私聊添加指定推送目标"),
            ("ai推送删除 <群|私聊> <ID>", "从任意群或私聊删除指定推送目标"),
            ("ai推送开启 / ai推送关闭", "不带参数时开启或关闭当前群或私聊的推送，也可指定目标"),
            ("ai推送列表", "查看全部群与私聊推送目标"),
