...
知微（仓库名 `ayjx`）是一个跑在自己机器上的群聊机器人：基于 Rust，通过 Satori v1 协议连接实现端，23 个插件在配置文件里开关与调整。
...
名字取自《周易·系辞下》「君子知微知彰，知柔知刚，万夫之望」。它在群里做的两件事与这两个字对应：从一个人细小的行为里读出他稳定的样子，以及开口之前先听见话里没说出来的那半句。仓库名、可执行文件名与 `./bot` 脚本都还叫 `ayjx`，改的只是给人看的那个名字。

前一个名字是「知言」，取自《孟子·公孙丑上》。换掉它有三条理由，从重到轻：

1. **姿态反了。** 孟子那段把「知言」定义成挑出别人话里的毛病（「诐辞知其所蔽，淫辞知其所陷，邪辞知其所离，遁辞知其所穷」）——一个审视者的姿态。这个机器人在群里做的事情正好相反：它是被听、被评价「吵不吵」的那一个。
2. **「言」占的是宾语位。** 古义里「言」是「被知」的对象（《论语·尧曰》「不知言，无以知人也」），于是它最大宗的动作——自己开口说话——在名字里没有落点。
3. **同品类里已经有人叫这个。** 应用商店里有两个可下载的 AI 助手叫「知言」，另有两家做自然语言的公司同名。自己用不上架，这一条的代价只是撞衫，不是障碍。

「知微」对应它做的那两件事：从一个人细小的行为里读出他稳定的样子（「知微知彰」），开口之前先听见话里没说出来的那半句（「见微知著」）。

它与接入层那个模块的名字对应：QQ 进程里那个实现端模块叫[**知弦**](https://github.com/araea/satori-qq)（弦指把它接上），这里叫**知微**（微指它看的是那一点）。
