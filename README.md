...
「知微」取理解细微信息之意，对应搭话中的上下文理解与用户画像中的日常观察。它与接入层[知弦](https://github.com/araea/satori-qq)共同组成群聊机器人的两部分。仓库、可执行文件与 `./bot` 脚本继续使用 `ayjx`。本轮复审保留中文名与图标，完整依据见[名称与界面复审](docs/NAME_DESIGN_AUDIT.md)。
