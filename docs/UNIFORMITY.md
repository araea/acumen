...
| 界面拖慢机器人的风险 | 动画只碰 `transform` 与 `opacity`，外壳不设 `transform`/`filter`/`backdrop-filter`（那三样会让固定定位的底栏飞掉，也最费 GPU）；日志只在日志页可见时接收，服务端合批 50ms、一批最多 128 行，页面每 100ms 至多落一次 DOM，缓冲 2000 行、DOM 120 行；静态资源带 `ETag` 走 304 |
