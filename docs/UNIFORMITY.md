> 历史审查记录，不代表当前验收。现行契约见 [统一设计系统](DESIGN_SYSTEM.md)，2026-09-25 修复对应关系见 [升级记录](DESIGN_SYSTEM_UPGRADE.md)。

# 统一度审计（2026-09-16）

按十条硬条目与交互规范核对全仓 23 个插件，逐项整改。本文记整改之后的结论、每一项落在哪里，以及尚未处理的事项。
...
注册表、出图入口、配置写路径、图标集合这四项一直统一，且都有测试或类型约束。规范层此前是空的，于是同一件事在不同插件里有不同做法：把「超时」叫三个名字，把 `⚠️` 用来表达六种意思，把当前前缀写死进提示，两个出卡片的插件没有关图开关。本次补上这两层规范，并按它改到位。
...
**写配置只有一条路。** `/ctl`、agent 房间的 `acumen --ctl` 与本机控制台网页都汇到 `ctl::change`，先写盘再改内存。

**图标收敛。** 全仓只用到文案规范那六个加三个行内记号（`▍`、`🔗`、`📄`），没有装饰性表情。
...
**文案规范的边界清楚。** 文案规范写明不管搭话人格。

## 整改清单

### 第一轮：口径与硬条目

先定规则再改代码。日志 target 的旧规则与代码矛盾（举例 `Plugin/WordCloud`，但注册名是 `wordcloud`），口径改成「注册名按单词边界大写」，写进总纲，`ARCHITECTURE.md` 同步。指令本体里的数字不留空格（`近7天`）此前没写进规范，补进文案规范。状态图标与 koishi 工作区的两套不同，保留 acumen 这套（`📭` 代替 `📋`，没有 `🏆`），在文案规范里写明差异是有意的。
...
| `⚠️` 11 处表达 6 种意思 | 文案规范收窄成「做成了，但要打折着看」，代码改到只剩 3 处（切片截断、改用缓存列表、切全量池），其余改成 ✅ 或 ❌。`⚠️` 11 降到 3，`❌` 75 升到 91 |
...
| 引用驱动的有效期没有统一口径 | 定成「由插件自己存对应关系的两种统一留 30 天」，两处常量互相指认并指向交互规范，`video_parse` 的注册表说明写明有效期 |
...
文案规范新增「空不是故障」一条，把这条判据与两处实现写在一起。

### 第三轮：文案终扫

按文案规范的人称、标点、术语三节扫全仓的用户可见文案。
...
| 空态、状态词、图标另起一套的风险 | 沿用文案规范：空态 `📭`、「已启用／已停用／待重启」三组状态词、`label-*` 两档小字 |
...
两处仍靠人工核对：**界面的手感**（间距、折行、滚动）只能看图，`review-console.sh` 的产物是给人看的。**窄屏折行**只有两条硬规矩（键名不断词、日志正文整条落到第二行），其余按总纲的分工判断。
...
| 域名撞名 | 产品名从「知言」改成「知微」。审计出的两条硬理由：古义里「言」是宾语（别人的话），而它最吃重的一件事是自己开口说话；以及应用商店里已经有两个可下载的 AI 助手叫「知言」。理由与出处写在 `README.md`；仓库名、可执行文件名与线上的 `x-acumen-token` 请求头当时判为内部标识而未动，2026-09-20 连英文名一起统一为 `acumen` |
...
同轮另外核过、没有偏离的：注册表元数据、示例配置三处同步（`example_config_tests` 三条绿）、容器级 `#[serde(default)]` 加一份 `Default`、术语表（`群聊工具` 一条进文案规范）、图标集合、提示词不在文案规范管辖内（`CHAT_HINT` 与工具描述都在此列）。
...
后续复审发现：宽屏行点击仍会截获开关，指令表单与连接开关存在遗漏，日志离页不断连，暂停仍改 DOM。此次统一修复，并把日志改为服务端合批、可见页订阅和有界 DOM 窗口；补齐快照连续性、浅深色对比、48px 触控目标与焦点状态。**DOM 窗口在第七轮从 120 行收至 80 行，每次最多插入 40 行**，并移除 `content-visibility`。旧版 WebUI 审计文档已随界面重做移除。

线上样张脚本现为三条只读交互断言；更完整的写操作回归移至隔离的 `tests/console.cjs`，不再改变运行实例的插件开关。

同一天的后一轮复审又确认并修复十余项：设置页草稿误删连接、错误页缺少标题、焦点环被裁切、对比度不足、含特殊字符的口令无法连接日志流、旧 URL 口令覆盖新口令、`log_lines` 上限不一致，以及测试连接到真实实现端等。旧版 WebUI 审计文档已移除；当前设计规范见 [WebUI 设计系统](DESIGN_SYSTEM.md)。
