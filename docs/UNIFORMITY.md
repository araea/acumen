...
| **失败被包了一层软话** | `ctl` 的 12 处错误一律渲染成「操作未完成：…」，既没有 ❌，又用一层铺垫把事实推后。改成 `❌ {原因}`，原因本身就是事实＋出路。同一处另有两条措辞：「请指定插件」改成「没写插件名」 |
| **引导被当成失败** | `ctl` 的 `reset` 不带 `--confirm` 时提示「重置会覆盖现有值」，那不是失败，是按 CONTENT.md 该用 💡 的引导；`oai` 音乐/视频房间「没说想写什么歌」原本走 `❌ 对话失败：请说明…`，也改成 💡 引导（顺带不再把一个音乐房间叫成「对话」）。两处都补了断言 |
| `ai_news` 提取序号的三种报错没图标，且用了弯引号 | 补 ❌（统一在渲染处加一次），`无法识别序号“2x”` 改成「认不出「2x」这个序号」 |
...
前几轮改的都是文案与配置，这一轮查的是 `m3e.css` 文件头那条规则：
**版式层里不许出现色值、字号、圆角、阴影的字面量。**

| 项 | 处理 |
| --- | --- |
| `oai` 的回复卡在系统之外又养了一套字阶 | 页脚轨迹、来源序号、智能体小卡这些「卡上的小字」原本写着 10 / 10.5 / 11 / 11.5 / 13 / 13.5 / 14 七个值——比整支字阶（11 / 14）还密。一律落到 `--md-type-label-small-size`（11px）与 `--md-type-label-medium-size`（14px）。行内代码的 `0.86em` 留着：它相对父级字号，不属于这支字阶 |
| `ai_news` 的条色带有一个 `border-radius:3px` | 换成 `var(--md-shape-xs)`（4px） |

查下来干净的：五张卡与 `reading.css` 里**一个十六进制色值都没有**，
`box-shadow` 也全部走令牌。这条规则现在是真的，不是写着的。
