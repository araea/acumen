...
2. 用户**引用那条预览**并回复「视频」「原片」「下载」「文件」这类词，才真的去取片：
   ...
   上传给实现端、按 `send` 配的发法发出去（默认只发视频气泡），最后把本地那份删掉。
...
`send` 默认只发视频气泡，不发群文件：有些群不让普通成员发群文件，发文件那条腿必定
失败，实现端还会为它重试到超预算（3 次 / 45 秒），把整条取片流程一起拖垮。要群文件的
群再开成 `file` 或 `both`——`both` 时两条腿各自兜住，一条成了就算送到；只配一条腿时，
它自己的失败照旧往上报，群里能看到「没取到」。
