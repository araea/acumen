...
- 同一个动作在全部插件里同名同义。已经分好家的三组词：

  | 概念 | 用 | 不用 |
  | --- | --- | --- |
  | 房间级开关（oai 的后缀指令） | `房间?开` / `房间?关` | 启用 / 打开 / 关闭 |
  | 推送目标开关（ai_news） | `ai推送开启` / `ai推送关闭` | 启用 / 打开 |
  | 状态词（写进句子里） | 插件用「已启用 / 已停用」，推送目标用「已开启推送 / 未开启推送」 | 两组混着用 |

  后两组是一个概念的两种粒度：后缀指令只能给一个字，所以是 `?开`；
  独立指令是个动词，`ai推送开` 念着别扭，所以是 `开启`。

- 别名用 ` / ` 分隔写在同一条记录里；首个抬为主指令，其余降级为别名。
- **代码吃得下的写法，注册表里必须写全。** `commands` 是面向用户的指令表，
  用户在 `/help` 里看不到的写法等于不存在——`ai模型排行榜` 这种能用的名字
  没写进去，他就只能靠猜。注册表不要求与代码里的匹配表逐字对应
  （有的插件按组匹配、有的走正则），但**不许少写**。
- 别名是给打过一次旧写法的人用的，不再新增第二套叫法。留着 `x` 又新加 `x.了`，
  帮助里就同时出现两个名字，反而更难学。
...
- `enabled`——它的含义在 22 个插件里完全一样，写 22 遍不增加信息量。
- 不在 `config.toml` 里的持久化结构（`oai/types.rs` 的 `Config` 是
  `data/oai/config.json` 的形状，由 ctl 与房间指令维护，用户不手改）。
  它们的字段该有说明，但理由是可维护性，不是「用户要读」。
...
- [ ] 空态用的是 📭 不是 ❌；推送日志里冷群没数据记 `info!`，别混进 `warn!`
