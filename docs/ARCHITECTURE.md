...
所以出图后再解码一次、扫一遍内容边界，裁到「内容 + 一圈 12—48px 纸色」重新编码
（`trim_paper_ring`）；内容顶到画布边的那一侧就保持原样，成图不会比设定画布还大。
这一趟在手机上约 30—60ms，换的是成图大小跟着内容走。GIF 的单次处理上限为 256 帧和
合计 3200 万像素。缩放、拼图也在分配输出画布前检查尺寸。图片切分按相邻网格边界
分配余数，完整保留原图边缘。
