...
`[console]` 那个插件在回环地址上发一张网页，是知言唯一的一张界面：运行状况、插件开关与配置、搭话的运行时文本、实时日志，以及接入与全局设置。它同时是一份可以装到桌面的应用——手机与桌面浏览器都能把它加到主屏幕，装上之后没有地址栏，顶栏贴在状态栏下面。
...
- 动画只碰 `transform` 与 `opacity`，没有模糊与大面积重绘；窄屏顶栏的品牌让位给页名时换的是透明度，不是字号——滚动中改字号会每帧重排。
- 日志只在日志页可见时接收：换页、标签页切到后台、离开页面都断开，返回后由服务端补一份快照，后台不解析日志。
- 服务端把日志合批（首行之后 50ms、一批最多 128 行），日志页每 100ms 至多落一次 DOM，缓冲最多 2000 行、DOM 最多 120 行，屏外行用 `content-visibility` 跳过渲染。完整记录始终在终端日志里。
