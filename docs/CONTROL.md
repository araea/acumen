...
- 界面字号比卡片图小一档（收在 `res/console/app.css` 开头那组 `--md-type-*` 覆盖里），一屏能多放几行；卡片图那一套不动。
...
审美靠图，下限靠算：`python3 scripts/audit-contrast.py` 用真 Chromium 逐元素算一遍 WCAG 2.2 AA 里能算出来的那几条（1.4.3 对比度、1.4.11 控件边界、2.5.8 触控目标、可访问名），七个页面 × 三档宽度 × 明暗两档，不过就退 1。它对着本仓库正在跑的那一份跑，也可以把地址当第一个参数指到别处。

### 这一层的设计语言

四个来源各管一段，冲突时按下表从上往下裁决——**次序本身就是规范**，写在同一份样式的文件头（`res/console/app.css`）里：
...
视觉权威只有两处，都在代码里：卡片那一层是 `res/cards/m3e.css` 的文件头，界面这一层是 `res/console/app.css` 的文件头（令牌表、组件清单、七条纪律）。**没有第三份规范文档可读，也不要再建一份。**
