...
- 页面不加载任何外部资源，图标是内嵌的矢量与 PNG，样式与脚本编进二进制，完全离线也打得开。
- 界面字号比卡片图小一档（收在 `res/console/tokens.css` 里那十三级 `--md-type-*` 的覆盖档），一屏能多放几行；卡片图那一套不动。
- 动画只碰 `transform` 与 `opacity`，没有模糊与大面积重绘；窄屏顶栏的品牌让位给页名时换的是透明度，不是字号——滚动中改字号会每帧重排。曲线与时长取 M3 Expressive 官方给的那套「不能用弹簧时的等价曲线」：空间运动允许过冲，效果运动不过冲。
- 日志只在日志页可见时接收：换页、标签页切到后台、离开页面都断开，返回后由服务端补一份快照，后台不解析日志。连接真的断了（服务端回 401／503 这种非事件流响应）时按 2→4→8→16 秒退避自己重开，不靠浏览器的自动重连。
- 服务端把日志合批（首行之后 50ms、一批最多 128 行），日志页每 100ms 至多落一次 DOM。缓冲最多 2000 行，DOM 窗口 80 行，一次最多插 40 行——突发时单次任务的节点增删因此有上限。**屏外行不再用 `content-visibility` 跳过渲染**：那会让面板的 `scrollHeight` 比实际矮，贴底落在一个过时的高度上，表现就是「看不到最新那几行」。完整记录始终在终端日志里。
- 日志的跟随只按一个判据：面板贴在底部就是跟随，离开底部就是暂停，滚回底部自己恢复。暂停期间 DOM 一个字节都不动，新行只进缓冲，「回到最新」上写着攒了多少条。
- 静态资源带 `ETag`，重复打开走 304，不必重下一百多 KB；表情包图按内容摘要长缓存。

2026-09-16 在本机（Android/Termux，机器人由 runit 托管）各取 60 秒实测：没有界面时机器人 0.03% 单核，开着总览页 0.02%，开着日志页（长连接）0.05%；浏览器那一侧 0.70% 与 2.77%（headless 的基线本来就有，日志页多出来的是它自己在画行）。

界面截图不必对着手机做：`bash scripts/review-console.sh` 会把三种宽度下的七页拍成本地图片，落在 `${TMPDIR:-/tmp}/acumen-console`，并顺手跑三条只读交互断言（底部导航能换页、点行身进详情、破坏性操作弹自家对话框）。写操作、压力与前后台切换的回归在 `node tests/console.cjs`，用隔离数据跑，不动这个实例。它与 `scripts/review-cards.sh` 是同一套用法。

审美靠图，下限靠算：`python3 scripts/audit-contrast.py` 用真 Chromium 逐元素算一遍 WCAG 2.2 AA 里能算出来的那几条（1.4.3 对比度、1.4.11 控件边界、2.5.8 触控目标、可访问名、占位文字），七个页面 × 三档宽度 × 明暗两档，不过就退 1。它对着本仓库正在跑的那一份跑，也可以把地址当第一个参数指到别处。占位文字要单独算：它进不了文本节点，逐元素那套看不见它。

### 这一层的设计语言

四个来源各管一段，冲突时按下表从上往下裁决——**次序本身就是规范**，写在同一份样式的文件头（`res/console/tokens.css`）里：

```text
平台原生规范 > 可用性与无障碍 > 产品一致性 > M3E > Carbon > Miuix
```

M3 Expressive 给核心视觉语言（配色角色、十三级字阶、形状与高度级、状态层、两条运动轨道）；Apple HIG 管交互与平台行为（安全区、状态栏配色、原生对话框、iOS 那份图标、不小于 48px 的触控目标）；Carbon 补 Web 与响应式，以及日志工作区那种信息密度高的界面；Miuix 只做视觉精修（线性图标的粗细与圆头圆角、数显的行高），不引入规则。

界面这一侧分三层，**值只在前两层**：系统层 `res/cards/m3e.css`（与卡片图共用的一套），界面令牌层 `res/console/tokens.css`（界面这一份的取值：字阶档、外壳与组件几何、状态层、动效），版式层 `res/console/app.css`（只写选择器与 `var()`，一条令牌定义都没有）。三层按这个顺序拼成一张样式表，越靠下越具体。四条单测钉着：引用的令牌必存在、版式层不许新造令牌、令牌层不许出现颜色字面量、令牌层只定义自定义属性。

视觉权威只有三处，都在代码里：卡片那一层是 `res/cards/m3e.css` 的文件头，界面的令牌是 `res/console/tokens.css` 的文件头（令牌表、四个来源的分工与裁决次序），界面的组件是 `res/console/app.css` 的文件头（组件规范、七条纪律）。**没有第四份规范文档可读，也不要再建一份。**
