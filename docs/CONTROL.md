...
- 日志按帧合批写 DOM；标签页切到后台就不再动 DOM，只把行收进缓冲区；一屏最多画最近 500 行（缓冲区留 `log_lines` 行）。
- 静态资源带 `ETag`，重复打开走 304，不必重下一百多 KB。
...
界面截图不必对着手机做：`bash scripts/review-console.sh` 会把三种宽度下的七页拍成本地图片，落在 `${TMPDIR:-/tmp}/ayjx-console`，并顺手跑四条交互断言（底部导航能换页、行内开关不换页、点行身进详情、破坏性操作弹自家对话框）。它与 `scripts/review-cards.sh` 是同一套用法。
