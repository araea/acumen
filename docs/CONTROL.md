...
`[console]` 那个插件在回环地址上发一张网页，把四样东西放到一处：运行状况、插件开关与配置、搭话的运行时文本、实时日志。它是这台机器上唯一的一张界面：`./bot ui` 打开它，同一局域网里的浏览器打开的也是它。
...
页面上有五处：总览、插件（开关与全部配置项，就地改）、搭话（人格与档案可改、记忆与表情包库可看）、日志（实时流，按级别与关键字筛）、命令（一行 `/ctl`）。顶栏那个齿轮是接入与全局：连哪几个实现端、指令前缀、全局群黑白名单、浏览器路径——它们不在任何插件的配置里，`/ctl` 够不着，没有这一页就只能在停机时改文件。
