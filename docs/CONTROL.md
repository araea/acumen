...
`set` 会按真实配置类型校验。保存使用临时文件和原子替换；校验或写入失败不会留下部分修改。批量开关会先验证全部名称，再统一写入。敏感字段（包含 Cookie）在回复中隐藏。凭据请在本机配置，避免在群聊或 Agent 房间里发送原始值。

配置升级时只补缺失字段，不覆盖已有值，也不自动删除旧字段。首次启用会自动初始化；连接钩子在下次连接时执行，定时排期修改也在下次连接后生效。已开始的请求和任务不会因关闭开关而强制取消。
...
`[global_filter]` 是所有插件之前的独立过滤层，语义与插件 `channel` 不同：开启白名单时只允许名单内的群，空白名单禁止所有群，并忽略黑名单；只有关闭白名单时才检查启用的黑名单。两个开关都关闭则允许所有群。主动推送必须同时通过全局规则和插件名单。
...
信息卡出图成功后只发卡片图；出图失败时改发可复制、可读屏的等价文本。OAI 删除或清空会先显示影响范围，60 秒内重发同一条指令才执行；不重发即取消。
