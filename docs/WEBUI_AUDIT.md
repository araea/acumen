...
| 标记在 16px 下环宽 0.96 个设备像素 | 不足一个像素的那部分被抗锯齿摊成灰的，一缩小环就散，剩下的那点像个句号 | 环 6.5 → 8.5、点 12 → 10、外径 32 → 32.5，三个数按「16px 下环宽够一像素、点还留得住、环宽不超过点直径的四成半」一起定；判据写在 `make-icon.py` 的文件头 |
